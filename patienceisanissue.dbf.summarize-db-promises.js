...
mysql = require('mysql');
DBF = require('./patienceisanissue.dbf-setup.js');
...
var processDBFs = function(queryResults){ // Returns a promise that forces ALL dbfPromises to resomve before thening
  var dbfs = queryResults;
  return(Promise.all(dbfs.map(dbfToPromise)).then(processTables))
}

var processTables = function(results){ // Returns a promise that forces ALL table description Promise to resolve before thening
...
// to an array of objects containing table names for the dbf in the dbfObj
